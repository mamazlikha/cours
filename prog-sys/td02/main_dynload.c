...
static void Scan_Args(int argc, char *argv[])
{
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            char *tri_Table[500] = {"bubble","insertion","merge","quick"};
            for(int i = 0; i<4;i++){
                char My_library[50] = "libTri_";
                strcat(My_library, tri_Table[i]);
                strcat(My_library, "-dynamicLib.so");
                load_library(My_library);
